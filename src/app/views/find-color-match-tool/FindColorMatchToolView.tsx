  ...
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (msg.type === MAIN_TO_UI.FIND_COLOR_MATCH_RESULT) {
        ...
        setProgress(null)
        setIsLoading(false)
        setSelectionEmpty(msg.payload.entries.length === 0 && !progress)
      }
      ...
      if (msg.type === MAIN_TO_UI.FIND_COLOR_MATCH_APPLY_RESULT) {
        if (msg.result.ok) {
          const key = `${msg.result.nodeId}:${msg.result.variableId}`
          setAppliedKeys((prev) => new Set([...prev, entryKey(entries.find(
            (e) => e.found.nodeId === msg.result.nodeId
          )!)]))
        }
      }
